...
contract1 = Contract.create(name: "Trident Gum for Services Contract", status: "draft", maker_id: company1.id, taker_id: company2.id, maker_agreement: "#{company1.name} gives $30 Million dollars and 5 cents worth of trident layers to #{company2.name}", taker_agreement: "#{company2.name} will provide #{company2.name} with services.")
...
log1 = Log.create(action: "Added Contract", company_id: contract1.maker_id, contract_id: contract1.id, user_id: user1.id, comment_id: comment1.id)
...
log2 = Log.create(action: "Added Comment", company_id: contract1.maker_id, contract_id: contract1.id, user_id: user1.id, comment_id: comment2.id)
